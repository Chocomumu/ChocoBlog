## 内部宏定义
### \_TIME\_H\_
最外部的宏定义，是time.h文件的最外层判断，防止重复定义。

### __time32_t
long 类型参数（crtdefs.h中已定义）
### __time64_t
__int64（long long） 类型参数（crtdefs.h中已定义）
...
32系统中使用__time32_t
### clock_t
long 类型参数   typedef long clock_t;

### size_t和ssize_t
判断是否定义了，_WIN64下定义为int64，否则为int
### NULL
判断是否定义了，cpp下_WIN64下定义为0LL，否则为0，c中定义为((void *)0)
...
struct tm {
    ...
    int tm_wday;  
    int tm_yday;
    int tm_isdst; //是否夏时令 daylight saving time
};
```
### CLOCKS_PER_SEC
取值1000，cpu没秒的时钟周期数
